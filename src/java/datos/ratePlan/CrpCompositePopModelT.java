...
public class CrpCompositePopModelT extends Nodo implements ResultI{
    ...
    public int procesar(ArrayList<String> crp, int index) {
        ...
        for(int i=index; i<crp.size();i++) {
            ...
            if(crp.get(i).matches("(?s)name: (.*)")) this.name= crp.get(i).substring(6);
            else if(("usageChargePopModel oneTimePopModel recurringPopModel").contains(crp.get(i))){ this.popModelType=crp.get(i);
                popModel=true;
            }
            ...
            else if(crp.get(i).matches("(?s)lowerBound: (.*)")) this.lowerBound= crp.get(i).substring(12);
            else if(crp.get(i).matches("(?s)rumTierExpression: (.*)") && tierBasis) this.rumTierExpression= crp.get(i).substring(19);
            else if(crp.get(i).matches("(?s)balanceElementNumCode: (.*)") && balanceTierExpression) this.balanceElementNumCode= crp.get(i).substring(23);
            else if(crp.get(i).matches("(?s)balanceElementName: (.*)") && balanceTierExpression) this.balanceElementName= crp.get(i).substring(20);
            else if(crp.get(i).matches("(?s)rumName: (.*)") && priceTier) this.rumName= crp.get(i).substring(9);
            else if(crp.get(i).matches("(?s)enforceCreditLimit: (.*)") && priceTier) this.enforceCreditLimit= crp.get(i).substring(20);
            else if(crp.get(i).matches("(?s)distributionMethod: (.*)") && priceTier) this.distributionMethod= crp.get(i).substring(20);
            else if(crp.get(i).matches("(?s)currencyCode: (.*)") && priceTier) this.currencyCode= crp.get(i).substring(14);
            else if(crp.get(i).matches("(?s)applicableQuantity: (.*)") && priceTier) this.applicableQuantity= crp.get(i).substring(20);
            else if(("priceTierRange tierRange").contains(crp.get(i)) && priceTier){
                PriceTierRangeT resul;
                if(priceTierValidityPeriods.size()==0)
                    resul = new PriceTierRangeT(itemCount);
                else
                    resul = new PriceTierRangeT(itemCount,priceTierValidityPeriods.size()-1);
